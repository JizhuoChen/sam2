...
import os
import logging
import json  # ADDED for writing the IoU JSON
...
def single_frame_inference(
    ...
    Returns:
      final_mask_bin (np.ndarray of shape (H, W)): binary predicted mask.
      pil_image (PIL.Image.Image): the original loaded image, for optional re-saving.
    """
    ...
    return final_mask_bin, pil_image
...
def main():
    ...
    # ADDED: new argument for dumping low IoU examples
    parser.add_argument(
        "--low_iou_dir",
        type=str,
        required=True,
        help="Output directory to store (img, pred, gt) for examples with IoU < 0.8."
    )
    ...
    # Prepare the low-iou subfolders
    os.makedirs(os.path.join(args.low_iou_dir, "img"), exist_ok=True)
    os.makedirs(os.path.join(args.low_iou_dir, "pred"), exist_ok=True)
    os.makedirs(os.path.join(args.low_iou_dir, "gt"), exist_ok=True)
    ...
    # ADDED: Keep track of images with IoU < 0.8
    low_iou_mapping = {}
    ...
    for folder_name in folder_names:
        ...
        pred_mask_bin, pil_image = single_frame_inference(
        ...
        # Now load the GT mask and compute IoU
        ...
        # ADDED: If IoU < 0.8, save (img, pred, gt) + record in JSON
        if iou_val < 0.8:
            low_iou_mapping[folder_name] = float(iou_val)

            # Save original image
            img_save_path = os.path.join(args.low_iou_dir, "img", f"{folder_name}.png")
            pil_image.save(img_save_path)

            # Save predicted mask
            pred_save_path = os.path.join(args.low_iou_dir, "pred", f"{folder_name}.png")
            save_grayscale_mask(pred_save_path, pred_mask_bin)

            # Save ground truth mask
            gt_save_path = os.path.join(args.low_iou_dir, "gt", f"{folder_name}.png")
            # We can convert gt_bin back to uint8 for saving
            gt_uint8 = gt_bin.astype(np.uint8)
            save_grayscale_mask(gt_save_path, gt_uint8)
    ...
    # ADDED: If we have any low IoU results, save them to a JSON
    if low_iou_mapping:
        json_path = os.path.join(args.low_iou_dir, "low_iou.json")
        with open(json_path, "w") as f:
            json.dump(low_iou_mapping, f, indent=2)
        logging.info(
            f"Saved {len(low_iou_mapping)} low-IoU samples (<0.8) data to '{json_path}'."
        )
